		...
		try {
			...
			switch(command) {
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* CarlosX\n*YOUTUBE:*nop* wa.me/+558499034946\n*INSTA:* to sem aff kk\n\n\n*Digite *.* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'yt2mp3':
					...
					anu  =  aguardam  fetchJson ( `https: //. mhankbarbar tecnologia / api / yta url =? $ { args [ 0 ] } & apiKey = $ { apiKey } ` ,  { método : 'get' } )
					anu  =  aguardam  fetchJson ( `https: // mhankbarbar. moe / api / yta url =? $ { args [ 0 ] } & apiKey = $ { apiKey } ` ,  { método : 'get' } )
					if  ( anu . error )  return  reply ( anu . error )
					teks  =  `* Título *: $ { anu . title } \ n * Tamanho do arquivo *: $ { anu . tamanho do arquivo } `
					polegar  =  Await  GetBuffer ( Anu . polegar )
					break
				case 'ytsearch':
